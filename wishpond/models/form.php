<?php
  ...
  class Form
  {
    const POST_META_KEY                           = "wishpond_form";
    const POST_META_WISHPOND_ID_KEY               = "wishpond_form_id";
    const POST_META_WISHPOND_MARKETING_ID_KEY     = "wishpond_form_marketing_id";
    const POST_META_WISHPOND_IMAGE_KEY            = "wishpond_form_image";
    const POST_META_WISHPOND_APPID_KEY            = "wishpond_form_appid";
    const POST_META_WISHPOND_LPTYPE_KEY           = "wishpond_form_type";

    # Need to shorten it because weird things start happening beyond some length
    const POST_META_WISHPOND_DESC_KEY             = "wishpond_form_desc";
    ...
    public function update_values($args = array())
    {
      ...
      if(isset($args["landing_page_type"]) && $args["landing_page_type"] != "")
      { 
        $this->landing_page_type    = $args["landing_page_type"];
      }
    ...
    public static function sanitize($permalink)
    {
      if(preg_match("/[^A-Za-z_\-0-9]/i", $path)) {
        return "Please only use alphanumeric characters, dashes and underscores in the form url";
      }
      if(strlen($string) > 0) {
        return "Please only use alphanumeric characters, dashes and underscores in the form url";
      }
    ...
    */
    public function save()
    {
      ...
      $fields = array(
        ...
        'post_status'   =>  'publish',
        'post_type'     =>  'page',
        'page_template' =>  'templates/wishpond-form.php',
        'post_content'  =>  '<!-- Please do not edit this page directly. This page was created using the Website Forms plugin by Wishpond -->[wpsc id="'.$this->wishpond_id.'" width="100%"]'
      );
      ...
      else
      {
        ...
        $existing_post = Form::get_by_wishpond_id($this->wishpond_id);
    ...
    public static function load($post)
    {
      ...
      return new Form(array(
        "path"                  => $post->post_name,
        "wishpond_marketing_id" => get_post_meta($post->ID, Form::POST_META_WISHPOND_MARKETING_ID_KEY, true),
        "wishpond_id"           => get_post_meta($post->ID, Form::POST_META_WISHPOND_ID_KEY, true),
        "title"                 => $post->post_title,
        "image_url"             => get_post_meta($post->ID, Form::POST_META_WISHPOND_IMAGE_KEY, true),
        "description"           => get_post_meta($post->ID, Form::POST_META_WISHPOND_DESC_KEY, true),
        "facebook_app_id"       => get_post_meta($post->ID, Form::POST_META_WISHPOND_APPID_KEY, true),
        "landing_page_type"     => get_post_meta($post->ID, Form::POST_META_WISHPOND_LPTYPE_KEY, true),
    ...
    {
      return Form::load(get_post($wordpress_id));
    }
    ...
    public function get_by_wishpond_id($wishpond_id)
    {
      $args = array(
        ...
        'meta_query' => array(
          array(
            'key' => Form::POST_META_WISHPOND_ID_KEY,
            'value' => $wishpond_id,
            'compare' => '='
          )
        )
    ...
    public function store_meta($args = array())
    {
      ...
      $landing_page_type      = $args["landing_page_type"] != "" ? $args["landing_page_type"] : $this->landing_page_type;

      $facebook_app_id        = $args["facebook_app_id"] != "" ? $args["facebook_app_id"] : $this->facebook_app_id;

      add_post_meta($this->wordpress_post_id, Form::POST_META_KEY, true, true);

      if($wishpond_id  != "")
      {
        self::add_or_update_meta($this->wordpress_post_id, Form::POST_META_WISHPOND_ID_KEY, $wishpond_id);
      }

      if($wishpond_marketing_id != "")
      {
        self::add_or_update_meta($this->wordpress_post_id, Form::POST_META_WISHPOND_MARKETING_ID_KEY, $wishpond_marketing_id);
      }

      if($image_url != "")
      {
        self::add_or_update_meta($this->wordpress_post_id, Form::POST_META_WISHPOND_IMAGE_KEY, $image_url);
      }

      if($description != "")
      {
        self::add_or_update_meta($this->wordpress_post_id, Form::POST_META_WISHPOND_DESC_KEY, $description); 
      }

      if($landing_page_type != "")
      {
        self::add_or_update_meta($this->wordpress_post_id, Form::POST_META_WISHPOND_LPTYPE_KEY, $landing_page_type); 
      }
      
      if($facebook_app_id != "")
      {
        self::add_or_update_meta($this->wordpress_post_id, Form::POST_META_WISHPOND_APPID_KEY, $facebook_app_id); 
      }
    ...
    public static function get_all_posts()
    {
      $meta_query = array(
        array(
          'key' => Form::POST_META_KEY,
          'value' => true,
          'compare' => '='
        ),
        array(
          'key' => Form::POST_META_WISHPOND_LPTYPE_KEY,
          'value' => "forms",
          'compare' => '='
        )
      );
